...
bool Session::ProcessCommand()
{
	while ( m_packets.size() != 0 )
	{
		...
		case 'A':
		{
			...
			std::string message = std::to_string( m_roomNumber ) + str::msg::PLAYER_JOINROOM + m_server->m_rooms[ m_roomNumber ].GetName() + "\r\n";
			SendChat( message );
		...
		case 'J':
		{
			...
			if ( roomNum >= 1 && m_server->m_rooms.count( roomNum ) )
			{
				...
				{
					std::string message = std::to_string( roomNum ) + str::msg::PLAYER_JOINROOM + m_server->m_rooms[ roomNum ].GetName() + "\r\n";
					SendChat( message );
					// 채팅방 컨테이너에 자신을 넣고, 시스템 메시지 출력 후 장면 이동
